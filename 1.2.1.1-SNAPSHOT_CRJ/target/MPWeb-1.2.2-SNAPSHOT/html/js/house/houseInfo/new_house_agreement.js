...
//				console.log(start.toISOString(), end.toISOString(), label);
			...
			// 免租方式
			common.loadItem('FEERENT.TYPE', function(json) 
			{
				var html = "";
				for ( var i = 0; i < json.length; i++) 
				{
				...
				$('#freeType').append(html);
			},false);
			// 产权证明文件
			common.loadItem('PROPERTY.FILE', function(json) 
					{
				var html = "";
				for ( var i = 0; i < json.length; i++) 
				{
...
//				$('#lease_period').val(common.getWindowsData('houseSign').lease_period);
				...
				$('#propertMan').val(common.getWindowsData('houseSign').username);
				$('#wtname').val(common.getWindowsData('houseSign').username);
				$('#wtemail').val(common.getWindowsData('houseSign').email);
				$('#wtAddress').val(common.getWindowsData('houseSign').address);
//				$('#wtmobile').val(common.getWindowsData('houseSign').username);
//				$('#fooler').val(common.getWindowsData('houseSign').floor);
				...
				$('#wtUser_id').val(common.getWindowsData('houseSign').user_id);
				$('#user_mobile').val(common.getWindowsData('houseSign').mobile);
				$('#wtmobile').val(common.getWindowsData('houseSign').mobile);
				...
				house_sign.initRoomCnt(roomCnt);
				$('#room_pz_div').hide();
...
//			$('#signBaseInfo').addClass('in active');
		...
		{
			// 绑定事件
			$('#wtmobile').blur(function(){
				house_sign.changeMobile('wt')
			});
			$('#dlmobile').blur(function(){
				house_sign.changeMobile('dl')
			});
			$('#dlmobile,#dlname,#dlIDCard,#dlAddress,#dlUser_id').blur(function(){
				var dlmobile = $('#dlmobile').val();
				var dlname = $('#dlname').val();
				var dlIDCard = $('#dlIDCard').val();
				var dlAddress = $('#dlAddress').val();
				var dlUser_id = $('#dlUser_id').val();
				if(dlmobile == '' && dlname == '' && dlIDCard == '' && dlAddress == '' && dlUser_id == '')
				{
					 $('#dlmobile').removeAttr('dataType');
					 $('#dlname').removeAttr('dataType');
					 $('#dlAddress').removeAttr('dataType');
					 $('#dlUser_id').removeAttr('dataType');
					 $('#dlIDCard').removeAttr('dataType');
				}
				else
				{
					 $('#dlmobile').attr('dataType','Mobile');
					 $('#dlname').attr('dataType','Require');
					 $('#dlAddress').attr('dataType','Require');
					 $('#dlUser_id').attr('dataType','Require');
					 $('#dlIDCard').attr('dataType','IdCard');
				}
			});
		},
		changeMobile:function(prefix)
		{
			...
			if(!reginx.test(mobile))
			{
				if(mobile != '')
				{
					layer.tips('请输入11位数字格式的手机号码', $('#'+prefix+'mobile'),{
		                tips: [1, '#d9534f'] //还可配置颜色
		            });
				}	
				$('#'+prefix+'mobile').removeAttr("readonly","readonly");
				$('#'+prefix+'name').val('');
				$('#'+prefix+'IDCard').val('');
				$('#'+prefix+'email').val('');
				$('#'+prefix+'Address').val('');
...
//						console.log(data);
						if(data.state == 1)
						{
							...
							$('#'+prefix+'IDCard').val(data.certificateno);
							$('#'+prefix+'email').val(data.email);
							$('#'+prefix+'Address').val(data.desc_text);
							$('#'+prefix+'User_id').val(data.id);
						}
...
//			console.log($('#leaseTime option:selected').attr("data_rent"));
			...
			for(var i = 0; i < showL; i++)
			{
				...
				for(var j = 1; j < Math.abs(hs+1); j++)
				{
					...
					showHtml += '<input type="number" dataType="Integer3"  msg="请输入第'+tmpLength+'年房租！" maxlength="10" placeholder="第'+tmpLength+'年房租" class="form-control"  name="rentMonth" disabled/>&nbsp;&nbsp;';
				}	
				showHtml += '</div>';
			}
			if(syL > 0)
			{
				...
				for(var i = 0; i < 5; i++)
				{
					...
					{
						showHtml += '<input type="number" style="opacity: 0;" disabled="disabled" dataType="Integer3" msg="请输入第'+tmpLength+'年房租！" maxlength="10" placeholder="第'+tmpLength+'年房租" class="form-control"  name="rentMonth" disabled/>&nbsp;&nbsp;';
					}	
				}
				showHtml += '</div>';
			}
...
//				console.log(i); 
		...
		{
			var roomPz = '';
			$("input[name='room_pz_']").each(function(index,item){
				 roomPz += $(this).val();
				  });
			$('#oldMatched').val(roomPz);
...
//			$('#house_add_bnt').unbind("click");
			...
			$('#agentPath').val(path);
			var keys_count = "";
			keys_count = $('#doorkey').val()+"|"+$('#fooler').val()+"|"+$('#personCount').val();
			$('#keys_count').val(keys_count);
			var ajax_option =
			{
				url:"/agreementMge/UpdateEntrustAgreement.do",//默认是form action
...
//			console.log(roomp);
		...
		agreementInfo:function()
		{
			...
			//alert(agreementId);
			common.ajax({
				url:common.root+'/agreementMge/newAgreementInfo.do',
				data:{id:agreementId},
				dataType: 'json',
				loadfun: function(isloadsucc, data)
				{
					...
					{
						console.log(data);
						$('#propertyCode').val(data.propertyCode);
						$('#propertMan').val(data.propertyPerson);
						$('#co_owner').val(data.co_owner);
						$('#dlAddress').val(data.dlAddress);
						$('#dlIDCard').val(data.dlIDCard);
						$('#dlemail').val(data.dlemail);
						$('#dlmobile').val(data.dlmobile);
						$('#dlname').val(data.dlname);
						$('#houseId').val(data.house_id);
//						$('#wtrealAddress').val(data.wtrealAddress);
						$('#wtAddress').val(data.wtAddress);
						$('#wtIDCard').val(data.wtIDCard);
						$('#wtemail').val(data.wtemail);
						$('#wtmobile').val(data.wtmobile);
...
//							$('input[type="number"]').attr('disabled',true);
							...
							$("#descInfo").replaceWith("<span style='height: auto;' class='form-control'>"+data.desc_text+"</span>");
...
//						console.log("123data-->"+JSON.stringify(data))
						...
						$('#certificateno').val(data.certificateno);
						$('#wtIDCard').val(data.certificateno);
...
//						house_sign.editLoadMap(data.coordinate);
						...
						$('#houseType').val(data.specName+data.specName2);
