...
//							}
		...
		common.alert({
			...
			fun:function(action){
				if(action){
					...
					common.ajax({
					 	url:common.root+'/login/loginOut.do',
						dataType: 'json',
						loadfun: function(isloadsucc, data){
							if(isloadsucc){ 
								if(data.state == 1){
	...
	loadUser:function(){
		var this_ = this;
		common.ajax({
		 	url:common.root+'/login/loadUser.do',
			dataType: 'json',
			cache:true,
			loadfun: function(isloadsucc, data){
				if(isloadsucc){
	...
	setUser:function(this_){
		$('.index_user').text(this_.user.name);
	},
	...
	modifyPass:function(){
		$('.modify_pass').click(function(){
			common.openWindow({
			  name:'modifypass',
			  type: 1, 
			  url: common.root+'/html/modifypass.html'
			});
		});
	},
	/**
