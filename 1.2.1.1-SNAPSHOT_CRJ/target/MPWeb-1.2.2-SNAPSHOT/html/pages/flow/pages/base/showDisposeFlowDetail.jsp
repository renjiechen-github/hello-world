...
<body>
	<c:if test="${isMobile eq '1'}">
		...
		<input type="hidden" id="task_code" name="task_code"
			value="${map.taskdetail.task_code}">
		<input type="hidden" id="step_id" name="step_id"
			value="${map.stepMap.step_id}">
		<input type="hidden" id="cfg_step_id" name="cfg_step_id"
			value="${map.stepMap.cfg_step_id}">
		<input type="hidden" id="task_cfg_id" name="task_cfg_id"
			value="${map.taskdetail.task_cfg_id}">
		<div class="header">
			<div class="navdiv">
				<ul class="nav-pills">
					<div class="con right active" rl="b1">
						<a href="#"><li role="presentation">任务基本信息</li>
						</a>
					</div>
					<div class="con" rl="b2">
						<a href="#"><li role="presentation">任务处理信息</li>
						</a>
					</div>
				</ul>
			</div>
				...
				<div class="nummber">
					<c:if test="${not empty map.taskdetail.html_path }">
						<div class="panel panel-primary">
							...
							<jsp:include page="/flow/showTaskDetail.do?task_id=${map.taskdetail.task_id}&task_cfg_id=${map.taskdetail.task_cfg_id}&step_id=${map.stepMap.step_id}&cfg_step_id=${map.stepMap.cfg_step_id}&edit=1"></jsp:include>
						</div>
					</c:if>
				</div>
					...
					<div class="info">
						...
						<div class="content">
							...
							<div class="detail">
								...
								<div class="right" onclick="showOrHide('${s.count}')">
													...
													<c:choose>
														<c:when test="${fn:split(map2, '##')[2] == '1'}">
															...
															<a target="_blank" href="${task_path}${fn:split(map2, '##')[0]}">
																<img alt="" title="${fn:split(map2, '##')[1]}"
																src="${task_path}${fn:split(map2, '##')[0]}"> </a>
														</c:when>
														<c:otherwise>
															<br />
		...
		<script type="text/javascript" src="/client/js/xcConfirm/xcConfirm.js"></script>
		...
		<script type="text/javascript" src="/html/pages/flow/js/miBase/showDisposeFlowDetail.js"></script>
	...
	<c:if test="${isMobile eq '0'}">
		...
		<ul class="nav nav-tabs">
			<li class="active"><a data-toggle="tab" href="#home3"> <i
					class="fa fa-home"></i> 任务基本信息 </a></li>
			<li class=""><a data-toggle="tab" href="#about3"> <i
					class="fa fa-retweet"></i> 任务处理信息 </a></li>
			<!-- <li class="">
		...
		<div class="panel-body">
			<div class="tab-content">
				<div id="home3" class="tab-pane active">
					<form class="form-horizontal adminex-form " role="form">
						...
						<c:if test="${not empty map.taskdetail.file}">
							<div class="form-group col-sm-12">
								...
								<div class="col-sm-10">
									<div class="form-control-static">
										<c:forTokens items="${map.taskdetail.file}" delims=","
											var="map2">
											<c:choose>
												<c:when test="${fn:split(map2, '##')[2] == '1'}">
													<!-- 图片展示 -->
						...
						<c:if test="${not empty map.taskdetail.html_path }">
							<div class="panel panel-primary">
								...
								<div class="panel-body">
									<jsp:include
										page="/flow/showTaskDetail.do?task_id=${map.taskdetail.task_id}&task_cfg_id=${map.taskdetail.task_cfg_id}&step_id=${map.stepMap.step_id}&cfg_step_id=${map.stepMap.cfg_step_id}&edit=1"></jsp:include>
								</div>
							</div>
						</c:if>
						<%-- <div class="panel panel-primary">
	                        ...
	                        <div class="panel-body">
	                        	<div class="form-group col-sm-12">
									...
									<div class="col-sm-10">
										<select id="flowStepState" style="width: auto;" class="form-control m-bot15">
											<option value="" >请选择...</option>
			                                <option value="2" >完成当前步骤</option>
			                                <option value="3">回退</option>
			                                <option value="4">挂起当前步骤</option>
			                            </select>
			                            <div class="next_div hide" >下一步操作：
			                            	<c:forEach items="${stepList}" var="map3" >
			                            		${map3.step_name }
			                            	</c:forEach>
			                            </div>
									</div>
					...
					<br />&nbsp;
					<!-- <button id="flowsavebutt" onclick="showDisposeFlowDetail.save();" class="btn btn-success btn-lg btn-block" type="button">提交</button> -->
				</div>
				<div id="about3" class="tab-pane">
					<table id="detailFlowTable"
						...
						<tbody>
							<c:forEach items="${map.stepList}" var="map1" varStatus="s">
								<tr>
									...
									<td>
										<button
											onclick="showDisposeFlowDetail.detail(this,'${map1.step_id}','${map1.cfg_step_id}');"
											title="查看详细" class="button button-circle button-tiny">
											<i class="fa fa-plus"></i>
										</button></td>
								</tr>
								<tr class="hide" id="hide_flow_step_${map1.step_id}">
									<td colspan="9"><c:if test="${not empty map1.file}">
											<div class="form-group col-sm-12">
												<label class="col-sm-2  control-label">附件：</label>
		...
		<script type="text/javascript" src="/html/pages/flow/js/base/showDisposeFlowDetail.js"></script>
	</c:if>
</body>
</html>
