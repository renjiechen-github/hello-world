var page_num = 0;
var isLoading = false; 
var waiting_task_list = 
{
	init:function()
	{
		waiting_task_list.loadSelect();
		$('#rightdiv').click(function()
		{
			$('.myinfo').find('img').toggleClass('rot2');
			$('#hide1').toggle();
		});
		waiting_task_list.getMyTaskList();
...
//				$('#hide1').hide();
	...
	getMyTaskList:function()
	{
		...
		({
        	...
        	data:{taskType:$('#taskType').val(),pageNumber:page_num,keyWord:$('#keyWord').val(),lx:2,taskState:$('#taskState').val()},
        	dataType:'json',
        	async:false,
...
{
	njyc.phone.showShortMessage('数据加载完毕!');
	njyc.phone.openWebKit({title:'任务查看',url:njyc.phone.getSysInfo('serverPath')+'mobile/task/showDetailFlowDetail.do?step_id='+ encodeURIComponent(step_id)+'&task_id='+task_id});
}
//刷新列表
