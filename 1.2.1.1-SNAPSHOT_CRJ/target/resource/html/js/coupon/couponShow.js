var couponShow={
	init:function(){
		//获取数据
		var data = common.getWindowsData("couponShowDetail");
		...
		if(data.rule_least_cost != '' && data.rule_least_cost !=0){
//			$('#zdxf_c').click();
//			$('#rule_least_cost').val(data.rule_least_cost);
			$('#rowZdxf').append('最低消费 '+data.rule_least_cost+' 元可用');
		}
		if((data.reject_category != '') || (data.accept_category != '')){
			if(data.reject_category != ''){
				$('#rowSyfw').append('适用商品： '+data.reject_category);
			}
			if(data.accept_category != ''){
	...
	selectDegree:function(id){
		if(id == 1){
			$('#rule_degree_month').val(0).removeAttr('readonly');
		}else{
 				 ...
 				 if(a){
 					if(b.state == 1){
 						common.alert({
 							msg:'保存成功',
 							fun:function(){
 								common.closeWindow("couponShow", 3);
 							}
 						});
 					} else if(b.state == -4){
	...
	checkHy:function(){
		...
		if(ruledegree == undefined){
			...
			if(!Validator.isInteger2(rule_degree_month)){
				...
				if(parseInt(rule_degree_month) < 0 || parseInt(rule_degree_month) > 12){
