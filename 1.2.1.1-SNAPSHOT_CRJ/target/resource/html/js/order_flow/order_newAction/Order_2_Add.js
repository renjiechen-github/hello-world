...
var orderadd = {
	...
	initType : function() {
		...
		var param = {};
		common.ajax({
			url : common.root + '/cleaningOrder/getCleaningType.do',
			dataType : 'json',
      ...
      data : JSON.stringify(param),
			loadfun : function(isloadsucc, data) {
				if (isloadsucc) {
					...
					var date = data.cleaningTypeList;
	...
	save : function() {
		...
		// 保洁费用乘以100，传入后台
		var cost = parseInt($('#ocost').val()) * 100;
		...
		common.ajax({
			url : common.root + '/cleaningOrder/inputCleaningOrderInfo.do',
			dataType : 'json',
      contentType: 'application/json; charset=utf-8',
      encode: false,
      data : JSON.stringify(param),
			loadfun : function(isloadsucc, data) {
				...
				if (isloadsucc) {
					if (data.state == 1) {// 操作成功
						...
						common.closeWindow('orderadd2', 3);
