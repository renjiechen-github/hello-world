...
var dealerId;
var isLeader;
var cleaningOrderValueMap = {};
var jsCleaningOrderDetail = {
		...
		common.ajax({
			...
			loadfun: function(isloadsucc, data) {
				...
				if (isloadsucc) {
					...
					// 看房订单转成MAP
					jsCleaningOrderDetail.createCleaningOrderValueMap(respData);
					// 赋值父工单信息
					jsCleaningOrderDetail.generatorData(respData);
					// 赋值各流程信息
					jsCleaningOrderDetail.generatorHtml(respData);
					// 初始化各TAB页
					jsCleaningOrderDetail.initTab(respData);
				}
			}
		});
	},
	...
	// 看房订单转成MAP
	createCleaningOrderValueMap: function(data) {
		var valueList = data.subOrderValueList;
		for (var i = 0; i < valueList.length; i++) {
			var cleaningOrderValue = valueList[i];
			var key = cleaningOrderValue.attrPath;
			cleaningOrderValueMap[key] = cleaningOrderValue;
		}
	},
	...
	generatorData: function(data) {
		...
		var cost = parseInt(data.cleaningMoney) / 100;
		$('.js-cost').html(cost + "元");
		...
		if (isMobile == "Y") {
			...
			if (picurls != '' && picurls != 'null' && picurls != null && picurls != undefined && picurls != 'undefined') {
				njyc.phone.showPic(jsCleaningOrderDetail.changeImagePath(data.imageUrl), 'js-mobile-upurl');
	...
	initTab: function(data) {
		switch (data.state) {
			case subOrderStateDef.ASSIGN_ORDER: // 客服派单
				...
				break;
			case subOrderStateDef.REASSIGNING: // 重新指派
					...
					common.ajax({
						...
						loadfun: function(isloadsucc, data) {
							if (data.state == 1) {
								...
								common.ajax({
									url: common.root + '/cleaningOrder/reassignOrder.do',
									dataType: 'json',
									contentType: 'application/json; charset=utf-8',
									encode: false,
									data: JSON.stringify(param),
									loadfun: function(isloadsucc, data) {
										common.load.hide();
										jsCleaningOrderDetail.closeOpenedWin(isloadsucc, data);
									}
								});
							}
						}
	...
	customerServAssignHtml: function(data) {
		...
		// 获取供应商信息
		jsCleaningOrderDetail.ajaxManagerInfo('managerSelect', '28');
		...
		// 赋值客服订单说明
		if (data.attrCatg.code == 'CLEANING_ORDER_PROCESS') {
			for (var i = 0; i < data.attrCatg.attrCatgChildren.length; i++) {
				...
				if (childCatg.code == 'CUSTOMER_SERV_ASSIGN') {
					...
					for (var j = 0; j < childAttrList.length; j++) {
						var defaultPath = "CLEANING_ORDER_PROCESS.CUSTOMER_SERV_ASSIGN.";
		...
		var text = cleaningOrderValueMap['CLEANING_ORDER_PROCESS.CUSTOMER_SERV_ASSIGN.COMMENTS'];
	...
	butlerTakeOrderHtml: function(data) {
		...
		if (data.attrCatg.code == 'CLEANING_ORDER_PROCESS') {
			for (var i = 0; i < data.attrCatg.attrCatgChildren.length; i++) {
				...
				if (childCatg.code == 'SUPPLIER_TAKE_ORDER') {
					var childAttrList = childCatg.attrChildren;
					for (var j = 0; j < childAttrList.length; j++) {
						var defaultPath = "CLEANING_ORDER_PROCESS.SUPPLIER_TAKE_ORDER.";
						var childAttr = childAttrList[j];
						var attrPath = defaultPath + childAttr.code;
						childAttr.attrPath = attrPath;
						if (childAttr.code == 'COMMENTS') {
		...
		var text = cleaningOrderValueMap['CLEANING_ORDER_PROCESS.SUPPLIER_TAKE_ORDER.COMMENTS'];
		if (text != '' && text != 'null' && text != null && text != undefined) {
			text = text.textInput;
			// 设置备注回显
	...
	butlerExecuteHtml: function(data) {
		if (data.attrCatg.code == 'CLEANING_ORDER_PROCESS') {
			for (var i = 0; i < data.attrCatg.attrCatgChildren.length; i++) {
				...
				if (childCatg.code == 'SUPPLIER_EXECUTE') {
					var childAttrList = childCatg.attrChildren;
					for (var j = 0; j < childAttrList.length; j++) {
						var defaultPath = "CLEANING_ORDER_PROCESS.SUPPLIER_EXECUTE.";
						var childAttr = childAttrList[j];
						var attrPath = defaultPath + childAttr.code;
						childAttr.attrPath = attrPath;
						if (childAttr.code == 'COMMENTS') {
		...
		// 设置备注回显
		var text = cleaningOrderValueMap['CLEANING_ORDER_PROCESS.SUPPLIER_EXECUTE.COMMENTS'];
		if (text != '' && text != 'null' && text != null && text != undefined) {
			text = text.textInput;
		...
		// 回显图片
		var picurls = cleaningOrderValueMap['CLEANING_ORDER_PROCESS.SUPPLIER_EXECUTE.HOUSE_PICTURE'];
		if (isMobile == "Y") {
			// $('#upurl-1').data('attr', childAttr);
			var imageUrl = "";
			...
			njyc.phone.showPic(jsCleaningOrderDetail.changeImagePath(imageUrl), 'js-mobile-upurl-1');
			if (data.state != 'D') {
			...
			if (data.state == subOrderStateDef.DO_IN_ORDER && resFlag == true) {
				jsCleaningOrderDetail.initPicurls(picurls, 'upurl-1', true);
			} else {
				jsCleaningOrderDetail.initPicurls(picurls, 'upurl-1', false);
			}
		}
	},
	...
	customerServVisitHtml: function(data) {
		if (data.attrCatg.code == 'CLEANING_ORDER_PROCESS') {
			for (var i = 0; i < data.attrCatg.attrCatgChildren.length; i++) {
				...
				if (childCatg.code == 'CUSTOMER_SERV_VISIT') {
					var childAttrList = childCatg.attrChildren;
					for (var j = 0; j < childAttrList.length; j++) {
						var defaultPath = "CLEANING_ORDER_PROCESS.CUSTOMER_SERV_VISIT.";
						var childAttr = childAttrList[j];
						var attrPath = defaultPath + childAttr.code;
						childAttr.attrPath = attrPath;
						if (childAttr.code == 'COMMENTS') {
		...
		var text = cleaningOrderValueMap['CLEANING_ORDER_PROCESS.CUSTOMER_SERV_VISIT.COMMENTS'];
		if (text != '' && text != 'null' && text != null && text != undefined) {
			text = text.textInput;
			// 设置备注回显
		...
		var picurls = cleaningOrderValueMap['CLEANING_ORDER_PROCESS.CUSTOMER_SERV_VISIT.HOUSE_PICTURE'];
		if (isMobile == "Y") {
			var imageUrl = "";
			if (picurls != '' && picurls != 'null' && picurls != null && picurls != undefined) {
				imageUrl = picurls.textInput;
			}
			njyc.phone.showPic(jsCleaningOrderDetail.changeImagePath(imageUrl), 'js-mobile-upurl-2');
			...
			if ((data.state == subOrderStateDef.STAFF_REVIEW || data.state == subOrderStateDef.WAIT_2_TACE) && resFlag == true) {
				jsCleaningOrderDetail.initPicurls(picurls, 'upurl-2', true);
			} else {
				jsCleaningOrderDetail.initPicurls(picurls, 'upurl-2', false);
			}
		}
	},
	...
	generatorJudgeHtml: function(data) {
		if (data.attrCatg.code == 'CLEANING_ORDER_PROCESS') {
			...
			// 图片回显
			if (isMobile == "Y") {
				njyc.phone.showPic(jsCleaningOrderDetail.changeImagePath(picurls), 'js-judge-audit-image-url-mobile');
				if (data.state != 'D') {
				...
				if (data.state == subOrderStateDef.DO_IN_ORDER && resFlag == true) {
					jsCleaningOrderDetail.initPicurls1(picurls, 'image-upurl-1', true);
				} else {
					jsCleaningOrderDetail.initPicurls1(picurls, 'image-upurl-1', false);
				}
			}
		}
	},
		...
		common.alert({
			...
			fun: function(action) {
				if (action) {
					...
					common.ajax({
						url: common.root + '/cleaningOrder/assignOrder.do',
						dataType: 'json',
						contentType: 'application/json; charset=utf-8',
						encode: false,
						data: JSON.stringify(param),
		...
		common.alert({
			...
			fun: function(action) {
				if (action) {
					...
					common.ajax({
						url: common.root + '/cleaningOrder/takeOrder.do',
						dataType: 'json',
						contentType: 'application/json; charset=utf-8',
						encode: false,
						data: JSON.stringify(param),
	...
	butlerAgainReassign: function() {
		...
		// 获取供应商信息
		jsCleaningOrderDetail.ajaxManagerInfo('butlerAgainReassignSelect', '28');
		...
		common.alert({
			...
			fun: function(action) {
				if (action) {
					...
					var pathe = "";
					if (isMobile == 'Y') {
						pathe = jsCleaningOrderDetail.getMobileImagePath($('#js-mobile-upurl-1 input[name="picImage"]'));
					} else {
						pathe = jsCleaningOrderDetail.getPicPath("upurl-1");
					}
					var param = {
						'code': respData.code,
						'dealerId': dealerId,
					...
					common.ajax({
						url: common.root + '/cleaningOrder/processOrder.do',
						dataType: 'json',
						contentType: 'application/json; charset=utf-8',
						encode: false,
						data: JSON.stringify(param),
	...
	reassignOrderInStaffReview: function() {
		...
		// 获取供应商信息
		jsCleaningOrderDetail.ajaxManagerInfo('managerSelect1', '28');
	...
	reassign: function() {
		...
		var pathe = "";
		if (isMobile == 'Y') {
			pathe = jsCleaningOrderDetail.getMobileImagePath($('#js-mobile-upurl-2 input[name="picImage"]'));
		} else {
			pathe = jsCleaningOrderDetail.getPicPath("upurl-2");
		}
		common.alert({
			msg: "确定重新指派给供应商（" + manager + "）吗？",
			confirm: true,
			fun: function(action) {
				if (action) {
					...
					common.ajax({
						url: common.root + '/cleaningOrder/reassignOrderInStaffReview.do',
						dataType: 'json',
						contentType: 'application/json; charset=utf-8',
						encode: false,
						data: JSON.stringify(param),
			...
			fun: function(action) {
				if (action) {
					...
					if (isMobile == 'Y') {
						...
						pathe = jsCleaningOrderDetail.getPath("upurl-2");
					}
					var param = {
						'code': respData.code,
						'dealerId': dealerId,
					...
					common.ajax({
						url: common.root + '/cleaningOrder/passInStaffReview.do',
						dataType: 'json',
						contentType: 'application/json; charset=utf-8',
						encode: false,
						data: JSON.stringify(param),
		...
		common.alert({
			...
			fun: function(action) {
				if (action) {
					...
					common.ajax({
						url: common.root + '/cleaningOrder/staffTace.do',
						dataType: 'json',
						contentType: 'application/json; charset=utf-8',
						encode: false,
						data: JSON.stringify(param),
		...
		if (isloadsucc) {
			if (data.state == 1) {
				...
				if (isMobile == 'Y') {
					...
					// common.closeWindow('cleaningOrderDetail', 3);
				}
			} else {
				common.alert({
					msg: common.msg.error
