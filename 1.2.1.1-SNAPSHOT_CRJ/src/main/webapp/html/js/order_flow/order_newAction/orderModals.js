...
var orderModal = {
	...
	table.init({
		...
		columns : [ "agree_code", "name", "username", "user_mobile","begin_time", "end_time"],
		param : function() 
		{
		var a = new Array();
		a.push({name : 'status',value : "2"});
		a.push({name : 'type',value : "2"});
		...
		{
			orderadd.getAgree(data.id,data.name,data.username,data.user_mobile);
			modals.modal('hide');
		},
