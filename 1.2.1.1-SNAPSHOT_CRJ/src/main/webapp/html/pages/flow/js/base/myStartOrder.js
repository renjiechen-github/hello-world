var jsWorkOrder = {
    ...
    loadData : function() {
        table.init({
            ...
            param: function(){
                var a = new Array();
                a.push({name: "currentDealerStartedOrder", value: 'Y'});
                ...
                return a;},
			aaSorting:[[5,"desc"]],
           	aoColumnDefs:[{ "bSortable": false, "aTargets": [ 0,1,2,3,4,6,7,8,9,10]}],
            bnt:[{
				 name:'查看详情',
							...
							if (isloadsucc)
							{
								sOut+='<tr><th>步骤名称</th>'
									+' <th>创建时间</th>'
				             	    ...
				             	    +' <th>执行时间</th>'
				             	    +' </tr>';
				                var resData=jsWorkOrder.typeSwitch(aData.type,data);
								var date="";
								var length= resData.length;
							   	for (var i = 0; i < length ; i++){
							   		...
							   		sOut+='<tr>'
							   			+' <td>'+resData[i].stateName+'</td>'
							   			+' <td>'+date+'</td>'
						             	+' <td>'+resData[i].actualDealerName+'</td>'
						             	+' <td>'+resData[i].updateDate+'</td>'
						             	+' </tr>';
			...
			createRow:function(rowindex,colindex,name,value,data,row){
				...
				if(colindex == 1)//处理当前处理人判断
				 {
					return jsWorkOrder.dealColum({"value":value,"length":8});
				 }
			}
         });
    },
