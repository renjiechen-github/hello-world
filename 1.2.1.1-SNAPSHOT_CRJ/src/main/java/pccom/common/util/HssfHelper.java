...
public class HssfHelper {
	...
	public static void main(String args[]) throws Exception {
		...
		//logger.debug("BOLD:" + HSSFFont.BOLDWEIGHT_BOLD);
		wb.write(fileOut);
		fileOut.close();
	}
...
//        logger.debug("插入语句： " + insertSql);
        ...
        else 
        {
            ...
            // 删除临时表
            logger.debug("临时表：" + tempTableName);
     ...
     * @param startRow		从Excel第几行开始导入
     * @param orderFlag		顺序标志，如果为true，临时表中增加show_order字段，遍历excel时插入数据，输出结果文件时用于排序
     * @return 1：导入并解析成功  0：SQL语句执行失败  -1：导入失败（Excel数据问题或执行存储过程出现异常） -2：Excel读取错误！ -3：未读取到数据
     */
