...
import com.ycdc.appserver.bus.controller.house.HouseMgrController;

public class AES_CBCUtils {
	public static final Logger logger = org.slf4j.LoggerFactory.getLogger(AES_CBCUtils.class);
	...
	public static String encrypt(String password, byte[] content) throws Exception {
		Cipher cipher = initCipher(password, Cipher.ENCRYPT_MODE);
		byte[] encrypted = cipher.doFinal(content);
		return Base64.encodeBase64String(encrypted);
	}

	public static String encrypt(Cipher cipher, byte[] content) throws Exception {
		byte[] encrypted = cipher.doFinal(content);
		return Base64.encodeBase64String(encrypted);
	}

	public static String decrypt(String password, String strIn) throws Exception {
	...
	public static void main(String[] args) throws Exception {
		...
		logger.debug("1000000加密时长 " + (System.currentTimeMillis() - aaa));
		logger.debug("加密时长 " + (System.currentTimeMillis() - aaa));
		logger.debug("原文：" + Code);
		logger.debug("密钥：" + password);
		logger.debug("密文：" + codE);
		logger.debug("解密：" + decrypt(password + "2", codE));
		...
		logger.debug(new String(text.getBytes("GBK"), "UTF-8"));
	}
}
