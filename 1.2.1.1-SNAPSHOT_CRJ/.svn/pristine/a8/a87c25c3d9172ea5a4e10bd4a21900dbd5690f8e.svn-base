...
public class Res {
	public static final Logger logger = org.slf4j.LoggerFactory.getLogger(Res.class);
...
//				this.update(sql);
				...
				for(int i=0;i<list.size();i++){
					...
					//实际需要支付金额
					String paycost = String.valueOf(Integer.valueOf(cost_a)*Integer.valueOf(pay_type));
					
					
					//循环处理数据
					int cnt = (("".equals(pay_type)||"0".equals(pay_type))?0:Integer.valueOf(months)/Integer.valueOf(pay_type))+1;
					//logger.debug(months+"--"+pay_type+"--"+cnt);
					int je = 0;//初次的金额
					for(int j=0;j<cnt;j++){
						if(Float.valueOf(je+"") >= Float.valueOf(cost)){
							continue;
						}
						//logger.debug("j:"+j);
						je += Integer.valueOf(paycost);
						sql = "SELECT a.`id` FROM financial_receivable_tab a  WHERE a.`secondary` = ? ORDER BY a.`plat_time` LIMIT ?,1";
						String receivable_id = this.queryForString(sql,new Object[]{id,j});
						if(!"".equals(receivable_id)){
							this.update("UPDATE financial_receivable_tab a SET a.`status` = 1 , a.`pay_time` = a.`plat_time` WHERE a.id = ?",
...
//				this.update(sql);
				...
				String sql = "INSERT INTO financial_settlements_tab"+
						" SELECT a.id,1,CONCAT(a.`name`,'_',a.sbcode),0,0,a.create_time,'',(CASE WHEN a.`status` = 2 THEN 1 ELSE 0 END),NULL FROM yc_agreement_tab a WHERE a.`type` = 2 AND a.`isdelete` = 1 AND a.`status` IN(2,3)";
				this.update(sql);
				...
				sql = "SELECT " +
						...
						"FROM " + 
						"  yc_agreement_tab a " + 
						"WHERE a.`type` = 2 AND a.`isdelete` = 1 AND a.`status` IN(2,3)  ";
				...
				for(int i=0;i<list.size();i++){
					...
					for(int j=0;j<cnt;j++){//根据需要插入的条数进行循环
						logger.debug("开始插入：：：："+agreement_id);
						//强字符串日期改成data
...
//						}
						...
						if(j == 0){
							this.update("INSERT INTO financial_receivable_tab(`name`,`correlation`,`secondary_type`,`secondary`,`category`,`create_time`,`cost`,`plat_time`,`update_time`,`target_bank`,`target_user`,`target_type`,`operator`,`remarks`,`status`,`BankSlip`,start_time,end_time)VALUES(?,?,?,?,?,NOW(),?,?,NOW(),?,?,?,?,?,1,?,DATE_SUB(DATE_FORMAT(?,'%Y-%m-%d'), interval ? day),DATE_FORMAT(?,'%Y-%m-%d'))",
							...
							this.update("INSERT INTO financial_receivable_tab(`name`,`correlation`,`secondary_type`,`secondary`,`category`,`create_time`,`cost`,`plat_time`,`update_time`,`target_bank`,`target_user`,`target_type`,`operator`,`remarks`,`status`,`BankSlip`,start_time,end_time)VALUES(?,?,?,?,?,NOW(),?,?,NOW(),?,?,?,?,?,1,?,DATE_FORMAT(?,'%Y-%m-%d'),DATE_FORMAT(?,'%Y-%m-%d'))",
