...
public class AmountUtil {
    ...
    public static Double divide(Double val1, Double val2, int scale) {
        System.out.println("pccom.common.util.AmountUtil.divide():"+val1+"-"+val2);
        if (null == val1) {
            val1 = new Double(0);
        }
        if (null == val2 || val2 == 0) {
        ...
        return new BigDecimal(Double.toString(val1)).divide(new BigDecimal(Double.toString(val2)),BigDecimal.ROUND_HALF_EVEN).setScale(scale, BigDecimal.ROUND_HALF_UP).doubleValue();
    }
     ...
     *
     * @param val
     * @param fmt NumberFormat currency = NumberFormat.getCurrencyInstance();
     * //建立货币格式化引用 NumberFormat percent = NumberFormat.getPercentInstance();
     * //建立百分比格式化引用
