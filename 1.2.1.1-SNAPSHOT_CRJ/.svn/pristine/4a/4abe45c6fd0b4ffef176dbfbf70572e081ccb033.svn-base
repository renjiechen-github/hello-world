var order5 = {
	...
	order5.createTable();
	//类型查询
	$("#order_status5").change(function() {
		  $('#search5').click();
	});
	$("#order_type5").change(function() {
		  $('#search5').click();
	});
    },
	/**
    ...
    initType: function(){
       //类型加载    	
        ...
        common.loadItem('ORDER.TYPE', function(json){
           ...
           for (var i = 0; i < json.length; i++)
           {
        	   if (json[i].item_id==7||json[i].item_id==8) 
        	   {
					continue;
			   }
			   html += '<option  value="' + json[i].item_id + '" >' + json[i].item_name + '</option>';
           }
           $('#order_type5').html(html);
       });
    },
    /**
     * 表格创建
    ...
    createTable: function()
    {
        table.init({
            id: '#order_table5',
            url: common.root + '/Order/shGetList.do',
            columns: [
                    "order_code",
					"typename",
					"order_name",
					...
					"statusname",
                     "oper_date",
					{name:"remark",isover:true,isshow:false,title:'派单说明'},
					{name:"stepremark",isover:true,isshow:false,title:'执行说明'},
					{name:"order_desc",isover:true,isshow:false,title:'订单详情'},
					{name:"create_time",isover:true,isshow:false,title:'创建时间'},
					{name:"name",isover:true,isshow:false,title:'执行人'},
					{name:"nowoper",isover:true,isshow:false,title:'当前修改人'},
					{name:"actiontime",isover:true,isshow:false,title:'当前修改时间'},
					//'<div class="btn-group"><button type="button"  class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">操作<span class="caret"></span></button><ul class="dropdown-menu ulClass"><li><a class="paddingClass" onclick="order5.view(this);"  href="#">查看详情</a></li></ul></div>'
					],
										...
										if (isloadsucc)
										{
											...
											for(var i=0;i<tdata.data.length;i++)
											{
												...
												html +='<tr num="0" class="odd">'
							                          ...
							                          +'onclick="order5.checkTask(\''+tdata.data[i].step_id+'\',\''+tdata.data[i].task_id+'\',\''+tdata.data[i].typenames+'\')" class=\"button button-circle button-tiny\"><i class=\"fa fa-edit\"></i></button></td >'
							                          +'</tr >';
											}
											html+='</tbody>';
										 }
			...
			{
                var a = new Array();
				a.push({name:'status',value:$('#order_status5').val()});
				a.push({name:'order_name',value:$('#order_name5').val()});
				a.push({name:'order_type',value:$('#order_type5').val()});
                return a;
		    },  
	            //默认按钮等操作
	            createRow:function(rowindex,colindex,name,value,data)
	            {
            	  if( colindex == 2)
				  {
					 return order5.dealColum({"value":value,"length":5});
				  }
	            }
        });	
    },
    ...
    view: function(obj)
    {
    	rowdata = table.getRowData('order_table5', obj);
    	var type=rowdata.order_type;
    	var orderName=rowdata.order_name;
		var url='/html/pages/order_flow/order_action/Order_'+type+'_view.html';
		var title =orderName;
