...
var workOrder;
var isMobile;
var livingProblemOrderValueMap = {};
var jsLivingProblemOrderDetail = {
    ...
    common.ajax({
      ...
      loadfun: function(isloadsucc, data) {
        if (isloadsucc) {
          ...
          // 看房订单转成MAP
          jsLivingProblemOrderDetail.createLivingProblemOrderValueMap(respData);
          // 赋值父工单信息
          jsLivingProblemOrderDetail.generatorData(respData);
          // 赋值各流程信息
          jsLivingProblemOrderDetail.generatorHtml(respData);
          // 初始化各TAB页
          jsLivingProblemOrderDetail.initTab(respData);
        }
      }
    });
  },
  // 看房订单转成MAP
  createLivingProblemOrderValueMap: function(data) {
    var valueList = data.subOrderValueList;
    for (var i = 0; i < valueList.length; i++) {
      var livingProblemValue = valueList[i];
      var key = livingProblemValue.attrPath;
      livingProblemOrderValueMap[key] = livingProblemValue;
    }
  },
  // 赋值父工单信息
  generatorData: function(data) {
    ...
    if (isMobile == "Y") {
      ...
      if (data.imageUrl != null && data.imageUrl != "") {
        njyc.phone.showPic(jsLivingProblemOrderDetail.changeImagePath(data.imageUrl), 'myAwesomeDropzone-view');
        $('#myAwesomeDropzone-view b').addClass('hidden')
      } else {
        $('#myAwesomeDropzone-view').remove();
      }
    } else {
      if (data.imageUrl != null && data.imageUrl != "") {
  ...
  initTab: function(data) {
    ...
    switch (data.state) {
      ...
      case subOrderStateDef.WAIT_2_TACE: // 客服回访待追踪
        ...
        break;
        /*
         * case subOrderStateDef.REASSIGNING_IN_STAFF_REVIEW: // 客服回访重新指派 // 显示按钮和备注
         * $('.js-tab >
         * li').find("a[href='#CUSTOMER_SERV_VISIT']").parent().addClass('active');
         * $('#CUSTOMER_SERV_VISIT').addClass('in active'); // 显示按钮 if (!resFlag) {
         * return; } $("#marketingExecutiveAuditPassBtn").removeClass("hidden");
         * $("#rentalAccountReturn").removeClass("hidden");
         * $("#reassignOrderBtn").removeClass("hidden");
  ...
  customerServAssignHtml: function(data) {
    ...
    // 赋值客服订单说明
    if (data.attrCatg.code == 'LIVING_PROBLEM_ORDER_PROCESS') {
      for (var i = 0; i < data.attrCatg.attrCatgChildren.length; i++) {
        ...
        if (childCatg.code == 'CUSTOMER_SERV_ASSIGN') {
          ...
          for (var j = 0; j < childAttrList.length; j++) {
            var defaultPath = "LIVING_PROBLEM_ORDER_PROCESS.CUSTOMER_SERV_ASSIGN.";
    ...
    var text = livingProblemOrderValueMap['LIVING_PROBLEM_ORDER_PROCESS.CUSTOMER_SERV_ASSIGN.COMMENTS'];
  ...
  butlerTakeOrderHtml: function(data) {
    // 赋值客服订单说明LIVING_PROBLEM_ORDER_PROCESS.CUSTOMER_SERV_ASSIGN.COMMENTS
    if (data.attrCatg.code == 'LIVING_PROBLEM_ORDER_PROCESS') {
      for (var i = 0; i < data.attrCatg.attrCatgChildren.length; i++) {
        var childCatg = data.attrCatg.attrCatgChildren[i];
        if (childCatg.code == 'BUTLER_TAKE_ORDER') {
          var childAttrList = childCatg.attrChildren;
          for (var j = 0; j < childAttrList.length; j++) {
            var defaultPath = "LIVING_PROBLEM_ORDER_PROCESS.BUTLER_TAKE_ORDER.";
            var childAttr = childAttrList[j];
            var attrPath = defaultPath + childAttr.code;
            childAttr.attrPath = attrPath;
            if (childAttr.code == 'COMMENTS') {
    ...
    var text = livingProblemOrderValueMap['LIVING_PROBLEM_ORDER_PROCESS.BUTLER_TAKE_ORDER.COMMENTS'];
    if (text != '' && text != 'null' && text != null && text != undefined) {
      text = text.textInput;
      // 设置备注回显
  ...
  butlerExecuteHtml: function(data) {
    if (data.attrCatg.code == 'LIVING_PROBLEM_ORDER_PROCESS') {
      for (var i = 0; i < data.attrCatg.attrCatgChildren.length; i++) {
        ...
        if (childCatg.code == 'BUTLER_EXECUTE') {
          var childAttrList = childCatg.attrChildren;
          for (var j = 0; j < childAttrList.length; j++) {
            var defaultPath = "LIVING_PROBLEM_ORDER_PROCESS.BUTLER_EXECUTE.";
            var childAttr = childAttrList[j];
            var attrPath = defaultPath + childAttr.code;
            childAttr.attrPath = attrPath;
            if (childAttr.code == 'COMMENTS') {
    ...
    var text = livingProblemOrderValueMap['LIVING_PROBLEM_ORDER_PROCESS.BUTLER_EXECUTE.COMMENTS'];
    if (text != '' && text != 'null' && text != null && text != undefined) {
      text = text.textInput;
      // 设置备注回显
      $('.js-rental-account-comments-textarea').text(text);
    }
    var imageObj = livingProblemOrderValueMap['LIVING_PROBLEM_ORDER_PROCESS.BUTLER_EXECUTE.WORK_IMAGE_URL'];
    if (isMobile == "Y") {
      $('#upurl').data('attr', childAttr);
      var imageUrl = "";
      ...
      njyc.phone.showPic(jsLivingProblemOrderDetail.changeImagePath(imageUrl), 'upurl-view');
      if (data.state != 'D') {
      ...
      jsLivingProblemOrderDetail.imageWork("upurl", clickEventOk, imageUrl);
    }
  },
  ...
  customerServVisitHtml: function(data) {
    if (data.attrCatg.code == 'LIVING_PROBLEM_ORDER_PROCESS') {
      for (var i = 0; i < data.attrCatg.attrCatgChildren.length; i++) {
        ...
        if (childCatg.code == 'CUSTOMER_SERV_VISIT') {
          var childAttrList = childCatg.attrChildren;
          for (var j = 0; j < childAttrList.length; j++) {
            var defaultPath = "LIVING_PROBLEM_ORDER_PROCESS.CUSTOMER_SERV_VISIT.";
            var childAttr = childAttrList[j];
            var attrPath = defaultPath + childAttr.code;
            childAttr.attrPath = attrPath;
            if (childAttr.code == 'COMMENTS') {
    ...
    var text = livingProblemOrderValueMap['LIVING_PROBLEM_ORDER_PROCESS.CUSTOMER_SERV_VISIT.COMMENTS'];
    if (text != '' && text != 'null' && text != null && text != undefined) {
      text = text.textInput;
      // 设置备注回显
      $('.js-marketing-executive-audit-comments-textarea').text(text);
    }
    var imageObj = livingProblemOrderValueMap['LIVING_PROBLEM_ORDER_PROCESS.CUSTOMER_SERV_VISIT.WORK_IMAGE_URL'];
    ...
    if (isMobile == "Y") {
      ...
      njyc.phone.showPic(jsLivingProblemOrderDetail.changeImagePath(imageUrl), 'js-visit-upurl-view');
      ...
      jsLivingProblemOrderDetail.imageWork("js-visit-upurl", clickEventOk, imageUrl);
    }
  },
  // 用户评价
  generatorJudgeHtml: function(data) {
    if (data.attrCatg.code == 'LIVING_PROBLEM_ORDER_PROCESS') {
      ...
      // 图片回显
      if (isMobile == "Y") {
        njyc.phone.showPic(jsLivingProblemOrderDetail.changeImagePath(picurls), 'js-judge-audit-image-url-mobile');
        if (data.state != 'D') {
        ...
        if (data.state == subOrderStateDef.DO_IN_ORDER && resFlag == true) {
          jsLivingProblemOrderDetail.initPicurls(picurls, 'image-upurl-1', true);
        } else {
          jsLivingProblemOrderDetail.initPicurls(picurls, 'image-upurl-1', false);
        }
      }
    }
  },
      ...
      fun: function(action) {
        if (action) {
          ...
          var param = {
            ...
            'dealerId': dealerId,
            'subOrderValueList': [{
              // 'livingProblemOrderId': respData.id,
          ...
          common.ajax({
            url: common.root + '/livingProblemOrder/assignOrder.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
            loadfun: function(isloadsucc, data) {
              common.load.hide();
              jsLivingProblemOrderDetail.closeOpenedWin(isloadsucc, data);
            }
          });
        }
      }
    ...
    common.alert({
      ...
      fun: function(action) {
        if (action) {
          ...
          common.ajax({
            url: common.root + '/livingProblemOrder/reassignOrder.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
    ...
    common.alert({
      ...
      fun: function(action) {
        if (action) {
          ...
          common.ajax({
            url: common.root + '/livingProblemOrder/takeOrder.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
      ...
      fun: function(action) {
        if (action) {
          ...
          if (isMobile == 'Y') {
            ...
            pathe = jsLivingProblemOrderDetail.getMobileImagePath($('#upurl-view input[name="picImage"]'));
          } else {
            upurl = $('#upurl').data('attr');
            pathe = jsLivingProblemOrderDetail.getPath("upurl");
          }
          var param = {
            'code': respData.code,
            'submitFlag': flag,
          ...
          common.ajax({
            url: common.root + '/livingProblemOrder/processOrder.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
      ...
      fun: function(action) {
        if (action) {
          ...
          if (isMobile == 'Y') {
            ...
            pathe = jsLivingProblemOrderDetail.getMobileImagePath($('#js-visit-upurl-view input[name="picImage"]'));
          } else {
            upurl = $('#js-visit-upurl').data('attr');
            pathe = jsLivingProblemOrderDetail.getPath("js-visit-upurl");
          }
          var param = {
            'code': respData.code,
          ...
          common.ajax({
            url: common.root + '/livingProblemOrder/reassignOrderInStaffReview.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
    ...
    if (isloadsucc) {
      if (data.state == 1) {
        ...
        if (isMobile == 'Y') {
          ...
          // common.closeWindow('livingProblemOrderDetail', 3);
        }
      } else {
        common.alert({
          msg: common.msg.error
    ...
    common.alert({
      ...
      fun: function(action) {
        if (action) {
          ...
          common.ajax({
            url: common.root + '/livingProblemOrder/passInStaffReview.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
    ...
    common.alert({
      ...
      fun: function(action) {
        if (action) {
          ...
          common.ajax({
            url: common.root + '/livingProblemOrder/staffTace.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
...
$(function() {
  jsLivingProblemOrderDetail.init();
});