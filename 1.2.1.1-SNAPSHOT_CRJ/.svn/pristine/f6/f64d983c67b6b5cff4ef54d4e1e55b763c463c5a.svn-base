package pccom.common.taglib.ui;
...
public class SelectTag extends BaseUITag
{
	...
	public int doStartTag () throws JspException
	{
		...
		else if (objList instanceof List)
		{
			...
			if (this.isDistinct()) obj_list = StringHelper.distinctList(obj_list, optionValue, optionOrder);
			Map map = null;
			this.printSelectStart();
			for (int i = 0; i < obj_list.size(); i++)
			{
				map = (Map) obj_list.get(i);
				String label = optionLabel == null ? "" : String.valueOf(map.get(optionLabel));
				String value = optionValue == null ? "" : String.valueOf(map.get(optionValue));
				String msg = optionMsg == null ? "" : String.valueOf(map.get(optionMsg));
				this.printOption(label, value, msg);
			}
		}
	...
	public void printSelectEnd ()
	{
		try
		{
			...
			if ("true".equals(readOnly))
			{
				...
				out.println("tar_" + id + ".onfocus = function(){var index" + id + " = tar_" + id
				            + ".selectedIndex; tar_" + id + ".onchange = function() { tar_" + id
				            + ".selectedIndex = index" + id + "; ui.alert('不可更改！');}}");
				out.println("</script>");
			}
			this.setId(null);
		}
	...
	public void printOption (String optionName, String optionValue, String msgValue)
	{
		try
		{
			out.print("<option value='" + optionValue + "' ");
			if (value != null && isSelected(optionValue))
			{
				out.print("selected=\"selected\"");
			}
			if (msgValue != null)
			{
				out.print("msg=\"" + msgValue + "\"");
			}
	...
	/**
	 * @description 判断是否需要默认选中
	 * @author 许正海 2013-08-30
	 * @param value
	 * @return
	 */
	public boolean isSelected (String value)
	{
		...
		for (String sValue : this.getValue().split(delims))
		{
	...
	public void printTagAttribute ()
	{
		...
		try
		{
			if (multiple != null) out.print("multiple=\"" + multiple + "\" ");
		}
		catch (IOException e)
		{
			e.printStackTrace();
