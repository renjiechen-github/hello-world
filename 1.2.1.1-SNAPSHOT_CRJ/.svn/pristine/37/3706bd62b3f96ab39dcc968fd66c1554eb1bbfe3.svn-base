...
var workOrder;
var isMobile;
var otherOrderValueMap = {};
var jsOtherOrderDetail = {
  // 初始化数据信息
  init: function() {
    ...
    common.ajax({
      ...
      loadfun: function(isloadsucc, data) {
        if (isloadsucc) {
          ...
          // 看房订单转成MAP
          jsOtherOrderDetail.createOtherOrderValueMap(respData);
          // 赋值父工单信息
          jsOtherOrderDetail.generatorData(respData);
          // 赋值各流程信息
          jsOtherOrderDetail.generatorHtml(respData);
          // 初始化各TAB页
          jsOtherOrderDetail.initTab(respData);
        }
      }
    });
  },
  ...
  // 看房订单转成MAP
  createOtherOrderValueMap: function(data) {
    var valueList = data.subOrderValueList;
    for (var i = 0; i < valueList.length; i++) {
      var otherOrderValue = valueList[i];
      var key = otherOrderValue.attrPath;
      otherOrderValueMap[key] = otherOrderValue;
    }
  },
  ...
  generatorData: function(data) {
    ...
    if (isMobile == "Y") {
      ...
      if (picurls != '' && picurls != 'null' && picurls != null && picurls != undefined && picurls != 'undefined') {
        njyc.phone.showPic(jsOtherOrderDetail.changeImagePath(data.imageUrl), 'js-mobile-upurl');
  ...
  initTab: function(data) {
    ...
    switch (data.state) {
      case subOrderStateDef.ASSIGN_ORDER: // 客服派单
        ...
        break;
      case subOrderStateDef.REASSIGNING: // 接单
      case subOrderStateDef.TAKE_ORDER: // 接单
      case subOrderStateDef.REASSIGNING_IN_STAFF_REVIEW: // 接单
        // 显示按钮和备注
  ...
  customerServAssignHtml: function(data) {
    ...
    // 赋值客服派单说明
    if (data.attrCatg.code == 'OTHER_ORDER_PROCESS') {
      for (var i = 0; i < data.attrCatg.attrCatgChildren.length; i++) {
        ...
        if (childCatg.code == 'CUSTOMER_SERV_ASSIGN') {
          ...
          for (var j = 0; j < childAttrList.length; j++) {
            var defaultPath = "OTHER_ORDER_PROCESS.CUSTOMER_SERV_ASSIGN.";
    ...
    var text = otherOrderValueMap['OTHER_ORDER_PROCESS.CUSTOMER_SERV_ASSIGN.COMMENTS'];
  ...
  butlerTakeOrderHtml: function(data) {
    // 赋值客服订单说明OTHER_ORDER_PROCESS.CUSTOMER_SERV_ASSIGN.COMMENTS
    if (data.attrCatg.code == 'OTHER_ORDER_PROCESS') {
      for (var i = 0; i < data.attrCatg.attrCatgChildren.length; i++) {
        var childCatg = data.attrCatg.attrCatgChildren[i];
        if (childCatg.code == 'BUTLER_TAKE_ORDER') {
          var childAttrList = childCatg.attrChildren;
          for (var j = 0; j < childAttrList.length; j++) {
            var defaultPath = "OTHER_ORDER_PROCESS.BUTLER_TAKE_ORDER.";
            var childAttr = childAttrList[j];
            var attrPath = defaultPath + childAttr.code;
            childAttr.attrPath = attrPath;
            if (childAttr.code == 'COMMENTS') {
    ...
    var text = otherOrderValueMap['OTHER_ORDER_PROCESS.BUTLER_TAKE_ORDER.COMMENTS'];
    if (text != '' && text != 'null' && text != null && text != undefined) {
      text = text.textInput;
      // 设置备注回显
  ...
  butlerExecuteHtml: function(data) {
    if (data.attrCatg.code == 'OTHER_ORDER_PROCESS') {
      for (var i = 0; i < data.attrCatg.attrCatgChildren.length; i++) {
        ...
        if (childCatg.code == 'BUTLER_EXECUTE') {
          var childAttrList = childCatg.attrChildren;
          for (var j = 0; j < childAttrList.length; j++) {
            var defaultPath = "OTHER_ORDER_PROCESS.BUTLER_EXECUTE.";
            var childAttr = childAttrList[j];
            var attrPath = defaultPath + childAttr.code;
            childAttr.attrPath = attrPath;
            if (childAttr.code == 'COMMENTS') {
    ...
    var text = otherOrderValueMap['OTHER_ORDER_PROCESS.BUTLER_EXECUTE.COMMENTS'];
    if (text != '' && text != 'null' && text != null && text != undefined) {
      text = text.textInput;
      // 设置备注回显
  ...
  customerServVisitHtml: function(data) {
    if (data.attrCatg.code == 'OTHER_ORDER_PROCESS') {
      for (var i = 0; i < data.attrCatg.attrCatgChildren.length; i++) {
        ...
        if (childCatg.code == 'CUSTOMER_SERV_VISIT') {
          var childAttrList = childCatg.attrChildren;
          for (var j = 0; j < childAttrList.length; j++) {
            var defaultPath = "OTHER_ORDER_PROCESS.CUSTOMER_SERV_VISIT.";
            var childAttr = childAttrList[j];
            var attrPath = defaultPath + childAttr.code;
            childAttr.attrPath = attrPath;
            if (childAttr.code == 'COMMENTS') {
    ...
    var text = otherOrderValueMap['OTHER_ORDER_PROCESS.CUSTOMER_SERV_VISIT.COMMENTS'];
    if (text != '' && text != 'null' && text != null && text != undefined) {
      text = text.textInput;
      // 设置备注回显
  ...
  generatorJudgeHtml: function(data) {
    if (data.attrCatg.code == 'OTHER_ORDER_PROCESS') {
      ...
      // 图片回显
      if (isMobile == "Y") {
        njyc.phone.showPic(jsOtherOrderDetail.changeImagePath(picurls), 'js-judge-audit-image-url-mobile');
        if (data.state != 'D') {
        ...
        if (data.state == subOrderStateDef.DO_IN_ORDER && resFlag == true) {
          jsOtherOrderDetail.initPicurls(picurls, 'image-upurl-1', true);
        } else {
          jsOtherOrderDetail.initPicurls(picurls, 'image-upurl-1', false);
        }
      }
    }
  },
    ...
    common.alert({
      ...
      fun: function(action) {
        if (action) {
          ...
          common.ajax({
            url: common.root + '/otherOrder/assignOrder.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
            loadfun: function(isloadsucc, data) {
              common.load.hide();
              jsOtherOrderDetail.closeOpenedWin(isloadsucc, data);
            }
          });
        }
      }
    ...
    common.alert({
      ...
      fun: function(action) {
        if (action) {
          ...
          common.ajax({
            url: common.root + '/otherOrder/reassignOrder.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
    ...
    common.alert({
      ...
      fun: function(action) {
        if (action) {
          ...
          common.ajax({
            url: common.root + '/otherOrder/takeOrder.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
    ...
    common.alert({
      ...
      fun: function(action) {
        if (action) {
          ...
          common.ajax({
            url: common.root + '/otherOrder/processOrder.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
    ...
    common.alert({
      ...
      fun: function(action) {
        if (action) {
          ...
          common.ajax({
            url: common.root + '/otherOrder/reassignOrderInStaffReview.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
    ...
    if (isloadsucc) {
      if (data.state == 1) {
        ...
        if (isMobile == 'Y') {
          ...
          // common.closeWindow('otherOrderDetail', 3);
        }
      } else {
        common.alert({
          msg: common.msg.error
    ...
    common.alert({
      ...
      fun: function(action) {
        if (action) {
          ...
          common.ajax({
            url: common.root + '/otherOrder/passInStaffReview.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
    ...
    common.alert({
      ...
      fun: function(action) {
        if (action) {
          ...
          common.ajax({
            url: common.root + '/otherOrder/staffTace.do',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            encode: false,
            data: JSON.stringify(param),
